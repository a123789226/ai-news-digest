...
func buildRequest(modelName string, candidates []pipeline.Candidate) requestEnvelope {
	...
	prompt := "Select the top 1 to 3 AI news items from the candidates below using this priority order: practical updates first, platform/model updates second, broader industry news third. Practical updates include API changes, SDKs, CLIs, developer tooling, coding assistants, agent frameworks, open-source releases, integrations, or changes to implementation workflow. Platform updates include model launches, API pricing, and rate-limit changes. If practical items are not enough, use platform updates and then broader news to fill up to 3 items. Return strict JSON with this exact shape: {\"items\":[{\"TitleEN\":\"...\",\"SummaryZH\":\"...\",\"WhyItMattersZH\":\"...\",\"Source\":\"...\",\"URL\":\"...\"}]}. Preserve the original English title. Write SummaryZH and WhyItMattersZH in Traditional Chinese. Avoid generic wording. Do not invent facts. If fewer than 3 strong items exist, return fewer. Candidates: " + string(candidateJSON)
